...
testHomework1 giverSig takerSig ctrRequirements startT endT wSlot = do
  ...
  checkBalance contractBalance $ do
    usp <- giverSig `spend` amount                                          -- NOTE creates a spending action, and
    giverSig `submitTx` vestingTx1 ctrRequirements usp amount               -- NOTE submit Tx with Giver's sig.
  ...
  checkBalance claimBalance $ do
    ...
    -- Possible BUG: shouldn't the tx be signed by the Taker?
